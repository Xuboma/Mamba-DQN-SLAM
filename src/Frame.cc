     ...
     mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
    ...
    //mnScaleLevels = mpORBextractorLeft->GetLevels();
    // mfScaleFactor = mpORBextractorLeft->GetScaleFactor();
    // cout<<"mnScaleLevels = mpORBextractorLeft->GetLevels();->->:::::"<< mpORBextractorLeft->GetLevels()<<endl;
    // cout<<"mfScaleFactor = mpORBextractorLeft->GetScaleFactor();->->:::::"<<mpORBextractorLeft->GetScaleFactor()<<endl;
    mnScaleLevels = mpORBextractorLeft->getNlevelss();
    mfScaleFactor = mpORBextractorLeft->getScaleFactors();
...
void Frame::UpdatePoseMatrices()  //更新位姿矩阵
{
    mRcw = mTcw.rowRange(0,3).colRange(0,3);   //mTcw为相机位姿
    mRwc = mRcw.t();
