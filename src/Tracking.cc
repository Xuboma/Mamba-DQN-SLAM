...
Tracking::Tracking(System *pSys,//系统实例
     ORBVocabulary* pVoc,//BOW字典
      FrameDrawer *pFrameDrawer,//帧绘制器
       MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath,//配置文件路径
...
bool Tracking::ParseCamParamFile(cv::FileStorage &fSettings)
{
    ...
    // Max/Min Frames to insert keyframes and to check relocalisation插入关键帧的最大最小帧数
    mMinFrames = 0;
    mMaxFrames = fps;
...
// 2.构建Frame
// 3.track()
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename, double &scaleFactors, int &nlevelss , float &nartioo)
{
    ...
    if(mImGray.channels()==3)
    {
        if(mbRGB)
            ...
            cvtColor(mImGray,mImGray,CV_BGR2GRAY);   //BGR
    }
    else if(mImGray.channels()==4)
    {
        if(mbRGB)
    ...
    {
        mpIniORBextractor->setScaleFactors(scaleFactors);
        mpORBextractorLeft->setScaleFactors(scaleFactors);
        mpIniORBextractor->setNlevelss(nlevelss);
        mpORBextractorLeft->setNlevelss(nlevelss);
    ...
    return mCurrentFrame.mTcw.clone();  //返回当前帧相机位姿
}
...
void Tracking::PreintegrateIMU()    // Perform preintegration from last frame
{
    
    std::cout << "xxxxxxxxxxxxxxxxxxxxxxstart preintegration ! xxxxxxxxxxxxxxxxxxxxxxx" << std::endl;
    ...
    mvImuFromLastFrame.clear();  
     //mlQueueImuData帧间的测量队列
    mvImuFromLastFrame.reserve(mlQueueImuData.size());  //reserve为容器预留足够的空间，避免不必要的重复分配，分配空间大于等于函数的参数
    ...
    while(true)
    {
        bool bSleep = false;
        {
            ...
            {
                IMU::Point* m = &mlQueueImuData.front();   //将mlQueueImuData第一个元素的引用赋值给m
                cout.precision(17);  //输出当前的浮点数的精度值，小数点后保留17位
    ...
    const int n = mvImuFromLastFrame.size()-1;  //mvImuFromLastFrame从前一帧到当前帧的IMU测量容器（由PreintegrateIMU填充）
    std::cout<<"print the value of n "<< n << std::endl;
    ...
    for(int i=0; i<n; i++)
    {
        ...
        if((i==0) && (i<(n-1)))
        {
            float tab = mvImuFromLastFrame[i+1].t-mvImuFromLastFrame[i].t;  //两帧之间的时间
            float tini = mvImuFromLastFrame[i].t-mCurrentFrame.mpPrevFrame->mTimeStamp;  //開始時間
        ...
        else if((i>0) && (i==(n-1)))
        {
            ...
            float tend = mvImuFromLastFrame[i+1].t-mCurrentFrame.mTimeStamp; //結束時間
            acc = (mvImuFromLastFrame[i].a+mvImuFromLastFrame[i+1].a-
        ...
        //进行预积分
        mpImuPreintegratedFromLastKF->IntegrateNewMeasurement(acc,angVel,tstep);  //从上一帧的IMU预积分
...
bool Tracking::PredictStateIMU()
{
    ...
    else if(!mbMapUpdated)   //mbMapUpdated为ture
    {
...
{
    const int N = vpFs.size();   //N为指向帧指针容器的大小
    vector<float> vbx;
    vbx.reserve(N);
    vector<float> vby;
    vby.reserve(N);
    ...
    cv::Mat grad  = cv::Mat::zeros(3,1,CV_32F);   //梯度
    for(int i=1;i<N;i++)
    {
        Frame* pF2 = vpFs[i];
        Frame* pF1 = vpFs[i-1];
...
void Tracking::Track(float nartio)
{
#ifdef SAVE_TIMES   //#ifdef判断某个宏是否被定义，若已定义，执行随后的语句
    mTime_PreIntIMU = 0;
    mTime_PosePred = 0;
    ...
    else   // Step 3：跟踪
    {
        ...
        std::chrono::steady_clock::time_point timeStartPosePredict = std::chrono::steady_clock::now();
            ...
            if(mState==OK)
            {
                ...
                else
                {
                    ...
                    bOK = TrackWithMotionModel(nartio);   //帧之间的相对变化是一样的
                    if(!bOK)
                        bOK = TrackReferenceKeyFrame();
                }
                ...
                if (!bOK)
                {
                    ...
                    else if(pCurrentMap->KeyFramesInMap()>10) //进入地图的关键帧数大于10
                    {
            ...
            else   //mState！=OK
            {
               ...
               // LOST：Atlas中新建地图
                if (mState == RECENTLY_LOST)
                {
                    ...
                    {
                        if(pCurrentMap->isImuInitialized())  //使用imu预测获得P，V，R
                            PredictStateIMU();
                        else  
                            bOK = false;
                ...
                else if (mState == LOST)  //对LOST状态处理
                {
                    ...
                    {
                        mpSystem->ResetActiveMap();   //如果地图中的关键帧数小于10,则重置激活地图
                        cout << "Reseting current map..." << endl;
                    }else
                        CreateMapInAtlas();  //在Atlas中建立新地图
            ...
            else
            {
                ...
                else   
                {
                   ...
                   //MM=Motion Model,通过运动模型进行跟踪的结果
                    ...
                    if(!mVelocity.empty())
                    {
                        bOKMM = TrackWithMotionModel(0.9);
                        // 这三行中的内容会在上面的函数中被更新
        ...
        // Step 4：在跟踪得到当前帧初始姿态后，现在对local map进行跟踪得到更多的匹配，并优化当前位姿
        ...
        mpFrameDrawer->Update(this);
        if(!mCurrentFrame.mTcw.empty())   //如果当前帧的位姿不为空
            mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.mTcw);

        if(bOK || mState==RECENTLY_LOST)
        {
            // Update motion model
            // Step 6：跟踪成功，更新恒速运动模型
            ...
            // Step 8：清除运动模型跟踪中 UpdateLastFrame中为当前帧临时添加的MapPoints（仅双目和rgbd）
            // 步骤7中只是在当前帧中将这些MapPoints剔除，这里从MapPoints数据库中删除
...
{
    if(mCurrentFrame.N>500)  //当前帧的关键点数目如果大于500帧设为第一个关键帧
    {   
        // 1.设置base坐标系，设置imu
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            /**
             * 
             * @param[in] sigma(第二个参数)          测量误差 1.0
             * @param[in] iterations(第三个参数)     RANSAC迭代次数，外部给定 200
             * */
...
//   如果找到足够的匹配项，我们将设置PnP求解器
    ...
    Optimizer::PoseOptimization(&mCurrentFrame);  //pnp方式求得位姿

    // Discard outliers
    // step 5：剔除优化后的outlier匹配点（MapPoints）
    //之所以在优化之后才剔除外点，是因为在优化的过程中就有了对这些外点的标记
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)  //mCurrentFrame.N当前帧关键点的数量
    {
 ...
 */
bool Tracking::TrackWithMotionModel(float nartioo)
{
    ORBmatcher matcher(nartioo,true);
   ...
   // Step 1：更新上一帧的位姿；对于双目或rgbd摄像头，还会根据深度值生成临时地图点
    ...
    // Project points seen in previous frame  上一帧的投影点
    int th;
 ...
 * 
 * Step 1：imu情况下，达到时间插入关键帧，纯VO模式下不插入关键帧，如果局部地图被闭环检测使用，则不插入关键帧
...
bool Tracking::Relocalization()
{
    ...
    const int nKFs = vpCandidateKFs.size();  //候选关键帧的数
...
{
    cout<<"Tracking::CreateNewMapPoints()"<<endl;
