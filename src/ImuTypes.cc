...
Preintegrated::Preintegrated(const Bias &b_, const Calib &a)
{
    Nga = a.Cov.clone();
    NgaWalk = a.CovWalk.clone();
    Initialize(b_);
}
...
{
    std::cout<<"XXXXXXXXXXXXXXX进入ReintegrateXXXXXXXXXXXXXXXX"<<std::endl;
    std::unique_lock<std::mutex> lock(mMutex);
    const std::vector<integrable> aux = mvMeasurements;
    Initialize(bu);  //bu为更新的偏置
...
void Preintegrated::IntegrateNewMeasurement(const cv::Point3f &acceleration, const cv::Point3f &angVel, const float &dt)
{
    ...
    std::cout<<"XXXXXXXXXXXXXXX进入IntegrateNewMeasurementXXXXXXXXXXXXXXXX"<<std::endl;
    ...
    // std::cout<<"the value of Wacc is "<<std::endl<<Wacc<<std::endl;
    ...
    //print the Nga and Ngwalk 
    // std::cout<<"the result of Nga is "<<std::endl<<Nga<<std::endl;
    // std::cout<<"the result of Ngawalk is "<<std::endl<<NgaWalk<<std::endl;

    // Update covariance
    C.rowRange(0,9).colRange(0,9) = A*C.rowRange(0,9).colRange(0,9)*A.t() + B*Nga*B.t();  //Nga为imu加和角速度噪声
    C.rowRange(9,15).colRange(9,15) = C.rowRange(9,15).colRange(9,15) + NgaWalk;   //Ngawalk为imu偏差的随机游走误差
    std::cout<<"the result of C is "<<std::endl<<C<<std::endl;
    ...
    // fout <<"the last result of C is "<<std::endl<< C <<std::endl;
    // cv::Mat testrow = C.rowRange(0,3).colRange(0,3).clone();   // 包括左边界，但不包括右边界
    // fout <<"the result of row is "<<std::endl<< testrow <<std::endl;
    // fout.close();
}

void Preintegrated::MergePrevious(Preintegrated* pPrev)
{
    std::cout<<"XXXXXXXXXXXXXXX进入MergePreviousXXXXXXXXXXXXXXXX"<<std::endl;
    if (pPrev==this)
        return;
...
{
    std::cout<<"XXXXXXXXXXXXXXX进入getinformationmatrixXXXXXXXXXXXXXXXX"<<std::endl;
    std::unique_lock<std::mutex> lock(mMutex);
    if(Info.empty())
    {
    ...
    std::cout<< "the info of matrix is " <<std::endl<<EI<<std::endl;
    return EI;
}
