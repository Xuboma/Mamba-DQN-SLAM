...
System::System(const string &strVocFile, //词典文件路径
    ...
    mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)),//空。。。对象指针？  TODO 
    mbReset(false),//无复位标志
...
//初始化局部建图线程并运行
    ...
    mpLocalMapper = new LocalMapping(this, mpAtlas,//指定使iomanip
         mSensor==MONOCULAR || mSensor==IMU_MONOCULAR, mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO, strSequence);
    //运行这个局部建图线程
    ...
    mpLoopCloser = new LoopClosing(mpAtlas,//地图
        mpKeyFrameDatabase,//关键帧数据库
        mpVocabulary, //ORB字典
        mSensor!=MONOCULAR);//当前的传感器是否是单目 // mSensor!=MONOCULAR);
         //创建回环检测线程
    mptLoopClosing = new thread(&ORB_SLAM3::LoopClosing::Run,//线程的主函数
         mpLoopCloser);//该函数的参数
    ...
    {//如果指定了，程序的运行过程中需要运行可视化部分
    	//新建viewer
        mpViewer = new Viewer(this, mpFrameDrawer,//帧绘制器
        mpMapDrawer,//地图绘制器
        mpTracker,//追踪器
...
//同理，输入为单目图像时的追踪器接口
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp,const vector<IMU::Point>& vImuMeas, string filename, double scaleFactors, int nlevelss,float nartioo)
{
    ...
    if (mSensor == System::IMU_MONOCULAR)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]);
            //获取相机位姿的估计结果
    cv::Mat Tcw = mpTracker->GrabImageMonocular(im,timestamp,filename,scaleFactors,nlevelss,nartioo);
    // cout << "获取相机位姿的估计结果--------------------------------------e"<<Tcw << endl;
    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState;
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;
...
//     else
//         Twb = vpKFs[0]->GetPoseInverse();  //4x4
...
//             cv::Mat Rwc = Tcw.rowRange(0,3).colRange(0,3).t();
//             cv::Mat twc = -Rwc*Tcw.rowRange(0,3).col(3);
//             vector<float> q = Converter::toQuaternion(Rwc);  //将旋转矩阵转换为四元数输出保存
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
    ...
    for(Map* pMap :vpMaps)
    {
        ...
        {
            if(!flag_1)
                if(pMap->GetAllKeyFrames().size()>2){
                    int r=1;
    ...
    if (mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO)
    {
        ...
        Twb = vpKFs[0]->GetPoseInverse();  //4x4
    // ofstream f_("f_dataset-MH01_mono.txt",ios::app);
    ofstream f_(filename.c_str(),ios::app);
    // ofstream tf_(filename.c_str(),ios::app);

    list<ORB_SLAM3::KeyFrame*>::iterator lRit;
    list<double>::iterator lT;
    list<bool>::iterator lbL;
    list<cv::Mat>::iterator lit;
    list<cv::Mat>::iterator lend;
    ...
    for(lit,lend;lit!=lend;lit++, lRit++, lbL++,lT++)
    {
        ofstream result("result.txt",ios::ate); //如果没有文件，生成空文件；如果有文件，清空该文件
        ofstream result_all("result_all.txt",ios::app);//如果没有文件，生成空文件；如果有文件，在文件尾追加
        if(*lbL)
            continue;
        KeyFrame* pKF = *lRit;
        cv::Mat Trw = cv::Mat::eye(4,4,CV_32F);
...
//        }
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO)
        {
            ...
            cv::Mat twb = -Rwb*Tbw.rowRange(0,3).col(3);
            vector<float> q = Converter::toQuaternion(Rwb);
            f_ << fixed<< setprecision(6) <<(*(lT)) << " " <<  setprecision(9) << twb.at<float>(0) << " " << twb.at<float>(1) << " " << twb.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
            // tf_ << setprecision(6) << 1e9*(*lT) << " " <<  setprecision(9) << twb.at<float>(0) << " " << twb.at<float>(1) << " " << twb.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
        }
        else
        {
            cv::Mat Tcw = (*lit)*Trw;
            ...
            cv::Mat twc = -Rwc*Tcw.rowRange(0,3).col(3);
            vector<float> q = Converter::toQuaternion(Rwc);  //将旋转矩阵转换为四元数输出保存
            f_ << fixed<< setprecision(6) <<(*(lT)) << " " <<  setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
//            result <<count<<" "<<fixed << setprecision(0) <<img_name<<" "<< fixed<< setprecision(6) <<(*(lT)) << " " <<  setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
            result <<count<<" "<<fixed << setprecision(0) <<1e9*(*(lT))<<" "<< fixed<< setprecision(6) <<(*(lT)) << " " <<  setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
//            result_all <<count<<" "<<fixed << setprecision(0) <<img_name<<" "<<fixed<< setprecision(6) <<(*(lT)) << " " <<  setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
            result_all <<count<<" "<<fixed << setprecision(0) <<1e9*(*(lT))<<" "<<fixed<< setprecision(6) <<(*(lT)) << " " <<  setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
            // tf_ << fixed<<setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2)<< endl;
        }
        //关闭文件输出流
        result.close();
        result_all.close();
...
void System::SaveTrajectoryEuRoC(const string &filename,const string &img_name)
{
    count++;
    vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    Map* pBiggerMap;
...
{
    cout << endl << "Saving keyframe trajectory to here " << filename << " ..." << endl;
