...
public:
    ...
    // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Returns the camera pose (empty if tracking fails).
    cv::Mat TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="",double scaleFactors=1.2, int nlevelss=8,float nartioo=0.6);
    ...
    void SaveTrajectoryEuRoC(const string &filename,const string &img_name);
    // void SaveTranslationEuRoC(const string &filename);
...
private:
    ...
    //追踪器。它接收帧并计算关联的相机位姿。它还决定何时插入新的关键帧，创建一些新的映射点，并在跟踪失败时执行重新定位。
    Tracking* mpTracker;
