...
class ObservationEmbeddingRepresentation(nn.Module):
    ...
    #     return obs_embed
    # 修改后的代码能让网络接受1*4096的数据
    # def forward(self, obs: torch.Tensor):
    def forward(self, obs: torch.Tensor):
        batch, seq = obs.size(0), obs.size(1)

        # 将输入数据调整为 [batch_size * seq, num_channels, height, width]
        obs = obs.view(-1, 1, 64, 64)

        obs_embed = self.observation_embedding(obs)

        obs_embed_flat = torch.flatten(obs_embed, start_dim=0, end_dim=1)
        num_features = obs_embed_flat.size(-1) // (batch * seq)

        obs_embed = obs_embed_flat.view(batch, seq, num_features)
    ...
    def make_image_representation(obs_dim: Tuple, outer_embed_size: int):
        ...
        flattened_size = compute_flattened_size(
            64, 64, kernels, paddings, strides
        )
        embedding = nn.Sequential(
            # Input 3 x 84 x 84
            nn.Conv2d(
