...
class ReplayBuffer:
    ...
    def __init__(
        ...
        if isinstance(env_obs_length, tuple):
            self.obss = np.full(
                [
                    self.max_size,
                    max_episode_steps,  # Keeps first and last obs together for +1
                    *env_obs_length,
                ],
                obs_mask,
                dtype=np.uint8,
    ...
    def store(
        ...
        if episode_idx >= self.max_size or obs_idx >= self.max_size:
            # Roll the arrays to move all elements forward
            self.obss[:-1, :-1] = np.roll(self.obss, -1, axis=(0, 1))[:-1, :-1]
            self.actions[:-1, :-1] = np.roll(self.actions, -1, axis=(0, 1))[:-1, :-1]
            self.rewards[:-1, :-1] = np.roll(self.rewards, -1, axis=(0, 1))[:-1, :-1]
            self.dones[:-1, :-1] = np.roll(self.dones, -1, axis=(0, 1))[:-1, :-1]

            # Set the last position to the new input
            episode_idx = self.max_size - 1
            obs_idx = self.max_size - 1
    ...
    #     obs_idx = self.pos[1] % self.max_size
    #     obs = obs.view(-1)
    #     self.obss[episode_idx, obs_idx] = obs
    #     self.actions[episode_idx, obs_idx] = action
    #     self.rewards[episode_idx, obs_idx] = reward
        ...
        if isinstance(self.env_obs_length, tuple):
            self.obss[episode_idx] = np.full(
                [
                    ...
                    ,  # Keeps first and last obs together for +1原来+1
                    *self.env_obs_length,
                ],
                self.obs_mask,
                dtype=np.uint8,
    ...
    def sample(
        ...
        # Generate episode indices in order
        episode_idxes = np.array(
            [[i] for i in range(self.max_size-1)]
        )
        # print(episode_idxes)
        # Generate transition start indices in order
        ...
        return (
            ...
            self.obss[episode_idxes, 1+transitions],
            self.actions[episode_idxes, 1+transitions],
            self.dones[episode_idxes, 1+transitions],
            np.clip(self.episode_lengths[episode_idxes], 0, self.context_len),
        )
