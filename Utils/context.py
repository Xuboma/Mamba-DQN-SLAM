...
class Context:
    ...
    def __init__(
        ...
        self.RNG = Generator(PCG64())  # 初始化随机数生成器
    ...
    def reset(self, obs: np.ndarray):
        ...
        # Initial observation
        self.obs[0] = obs
        # 用自己的随机数生成器每次抽取一个动作来使用
        self.action = self.RNG.integers(self.num_actions, size=(self.max_length, 1))
        # print(self.action)
    ...
    def add_transition(
        ...
        # self.obs[t] = o
        self.obs = self.obs.reshape(-1, 64, 64)
        self.obs[t] = o
